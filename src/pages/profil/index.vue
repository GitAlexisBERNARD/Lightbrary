<template>
  <div>
    ...
    <h2>{{ name }}</h2>
    <h2>Date de création du profil : {{ created }}</h2>
...
export default {
  ...
  methods: {
    calculateTopGenres() {
  let genreCounts = {};
  for (let category in this.genre) {
    if (Array.isArray(this.genre[category])) {
      this.genre[category].forEach((genre) => {
